...
class Settings {
    ...
    constructor(options) {
        ...
        this._frameEnd         = null;      
        this._infile           = null;      
        this._makeRenderScript = false;     
        this._outputBase       = "cpov0000";
        this._povrayPath       = null;      
        this._preamble         = null;      
    ...
    get makeRenderScript() {
        if(typeof this._makeRenderScript == "function")
            return this._makeRenderScript(cpov, this);
        else if(cpov.isSDLFunction(this._makeRenderScript))
            return this._makeRenderScript.substr(1);
        else
            return this._makeRenderScript;
    }

    set makeRenderScript(val) {
        if(cpov.isNullOrFunction(val) || (cpov.isBoolean(val))) {
            this._makeRenderScript = val;
        } else {
            cpov.error("fatal", "makeShellScript must be a boolean", "Settings", this);
        }
    }
    ...
    get povrayPath() {
        if(typeof this._povrayPath == "function")
            return this._povrayPath(cpov, this);
        else if(cpov.isSDLFunction(this._povrayPath))
            return this._povrayPath.substr(1);
        else
            return this._povrayPath;
    }

    set povrayPath(val) {
        if(cpov.isNullOrFunction(val) || (cpov.isNonEmptyString(val))) {
            this._povrayPath = val;
        } else {
            cpov.error("fatal", "povrayPath must be a non-empty string", "Settings", this);
        }
    }
    ...
    copy() {
        ...
        newObj.makeRenderScript = this.makeRenderScript;
        newObj.outputBase       = this.outputBase;      
        newObj.povrayPath       = this.povrayPath;      
...
class ImageOptions {
    ...
    output() {
        ...
        for(var i = 0; i < cpov.ioMutables.length; i++) {
    
            var opt = cpov.ioMutables[i];
