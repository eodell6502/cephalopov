...
class Settings {
    ...
    get optionalLibraries() {
        if(typeof this._optionalLibraries == "function")
            return this._optionalLibraries(cpov, this);
        else if(cpov.isSDLFunction(this._optionalLibraries))
            return this._optionalLibraries.substr(1);
        else
            return this._optionalLibraries;
    }

    set optionalLibraries(val) {
        if(cpov.isNullOrFunction(val) || (cpov.isArrayOfNonEmptyStrings(val) || (Array.isArray(val) && val.length == 0))) {
            this._optionalLibraries = val;
        } else {
            cpov.error("fatal", "optionalLibraries must be an array of non-empty strings or an empty array.", "Settings", this);
        }
    }
    ...
    copy() {
        ...
        newObj.optionalLibraries = this.optionalLibraries;
